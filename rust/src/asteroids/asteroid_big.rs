...
impl IArea2D for BigAsteroid {
    fn ready(&mut self) {
        ...
        self.rotation_speed = randi_range(1, 2) as i32;
        self.horizontal_speed = randi_range(15, 30) as f32 * self.direction.x;
        ...
        if self.main.get_name() == "Main".into() {
            self.camera_manager
                .init(self.main.get_node_as::<CameraManager>("CameraManager"));
        }
